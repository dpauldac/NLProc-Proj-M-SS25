...
class Evaluation:
    ...
    def evaluate_model_performance(self, test_report: Union[str, Path], threshold: float = 0.60):
        ...
        # new additinal
        # Group-level metric storage
        from collections import defaultdict

        group_metrics = {
            "source_type": defaultdict(
                lambda: {"precision": 0, "recall": 0, "f1": 0, "rouge": 0, "bert_f1": 0, "count": 0}),
        ...
        for key, item in test_report_data.items():
            ...
            #detailed_results.append((question, ground_truth, None, 0, 0, 0))  # F1, ROUGE, BERT F1
            ...
            question_pairs.append(
                (question, ground_truth, generated_answer, source_type, difficulty, question_type,  f1, rouge_score))  # BERT to be appended later
        ...
        for i, (question, ground, generated, source_type, difficulty, question_type, f1, rouge) in enumerate(question_pairs):
            bert_f1 = bert_f1_list[i]
            if bert_f1 >= threshold:
                matched += 1
            else:
                unmatched += 1
            detailed_results.append((question, ground, generated, source_type, difficulty, question_type, f1, rouge, bert_f1))

        # Added later
        # Update group-specific stats
        for category, value in [("source_type", source_type), ("difficulty", difficulty), ("question_type", question_type)]:
            group = group_metrics[category][value]
        ...
        #Added later
        for category in group_metrics:
            print(f"\nCategory: {category}")
            for value, metrics in group_metrics[category].items():
                count = metrics["count"]
                if count == 0:
                    continue
                ...
                print(
                    f"  {value}: P={avg_p:.2f}, R={avg_r:.2f}, F1={avg_f1:.2f}, ROUGE-L={avg_rouge:.2f}, BERTScore F1={avg_bert_f1:.2f}")

        output_dir = "evaluation_results"
        os.makedirs(output_dir, exist_ok=True)

        # Save group-level metrics to JSON
        ...
        return matched, unmatched, detailed_results, group_metrics
